...
public class OCParamHelper {
    ...
    static public boolean validate(OperatingConditionParameterInfo ocParam) {
        ...
        if(ocParam.workzone_length_long_to > defaultParam.workzone_length_long_to
            ...
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Corrupt configurations");
            return false;
        }
        
        if(ocParam.incident_downstream_distance_limit < 0) {
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Maximum distance from downstream boundary of a given route for incident\nmust be greater than or equal to 0");
            return false;
        }
        if(ocParam.incident_upstream_distance_limit < 0) {
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Maximum distance from upstream boundary of a given route for incident\nmust be greater than or equal to 0");
            return false;
        }        
        if(ocParam.incident_keep_in_minute < 0) {
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Affected Time by Incident must be greater than or equal to 0");
            return false;
        }        
        if(ocParam.workzone_length_short_from >= ocParam.workzone_length_short_to
            ...
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Wrong configurations in Workzone Length");
            return false;
        }          
        if(ocParam.workzone_downstream_distance_limit < 0) {
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Maximum distance from downstream boundary of a given route for workzone\nmust be greater than or equal to 0");
            return false;
        }     
        if(ocParam.workzone_upstream_distance_limit < 0) {
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Maximum distance from upstream boundary of a given route for workzone\nmust be greater than or equal to 0");
            return false;
        }             
        if(ocParam.specialevent_size_small_from >= ocParam.specialevent_size_small_to
            ...
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Wrong configurations in Special Event Size");
            return false;
        }            
        if(ocParam.specialevent_distance_near_from >= ocParam.specialevent_distance_near_to
            ...
            JOptionPane.showMessageDialog(TeTRESConfig.mainFrame, "Wrong configurations in Special Event Distance");
            return false;
        }    
        return true;
    }
