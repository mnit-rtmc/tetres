...
SET CREATE_DBS_SQL=%~dp0\Workspace\RunScripts\create_db.sql
...
"%PGHOME%\bin\initdb" -U postgres -A trust --encoding=UTF8 --locale=en_US
)^
ELSE (^
...
REM "%PGHOME%\bin\createdb" tetres -E UTF8 -T template0
REM "%PGHOME%\bin\createdb" cad -E UTF8 -T template0
REM "%PGHOME%\bin\createdb" iris -E UTF8 -T template0
REM "%PGHOME%\bin\psql" -U postgres -c ^"\set autcommit on;create database cad encoding UTF8 template template0;create database iris encoding UTF8 template template0;create database tetres encoding UTF8 template template0;\set autcommit off;^"
"%PGHOME%\bin\psql" -U postgres -f "%CREATE_DBS_SQL%"
)^
ELSE (^
