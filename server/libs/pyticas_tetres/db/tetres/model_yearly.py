...
def create_year_table(year):
    ...
    tt_table.__repr__ = lambda self: '<TravelTime%d id="%d" route_id="%d" time="%s" tt="%.1f" vmt="%.1f" speed="%.1f>' % (
    ...
    noaa_table = type('Noaa%d' % year, (Base,), {
        ...
        '__table_args__': (UniqueConstraint('usaf', 'wban', 'dtime', name='_noaa_uc_%d' % year),)
    })
    noaa_table.__repr__ = lambda self: '<NoaaWeather%d id="%d" usaf="%s" wban="%s" time="%s" precip="%s" precip_type="%s">' % (
    ...
    tt_weather_table.__repr__ = lambda self: '<TTWeather%d id="%d" tt_id="%d" weather_id="%d" precip="%s" precip_type="%s" precip_intensity="%s">' % (
    ...
    tt_workzone_table.__repr__ = lambda self: '<TTWorkZone%d id="%d" tt_id="%d" workzone_id="%d" ' \
                                              'loc_type="%d" distance="%.2f" off_distance="%.2f">' % (
                                                  year, self.id, self.tt_id, self.workzone_id, self.loc_type,
    ...
    tt_specialevent_table.__repr__ = lambda self: '<TTSpecialEvent%d id="%d" tt_id="%d" specialevent_id="%d" distance="%d" event_type="%s">' % (
    ...
    tt_snowmgmt_table.__repr__ = lambda self: '<TTSnowManagement%d id="%d" tt_id="%d" snowmgmt_id="%d" loc_type="%d" distance="%.2f" off_distance="%.2f" road_status="%d">' % (
        year, self.id, self.tt_id, self.snowmgmt_id, self.loc_type, self.distance, self.off_distance, self.road_status)
    ...
    tt_incident_table.__repr__ = lambda self: '<TTIncidentInfo%d id="%d" tt_id="%d" incident_id="%d" distance="%.2f" off_distance="%.2f">' % (
