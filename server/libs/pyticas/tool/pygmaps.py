...
class maps(object):
    ...
    def drawpoints(self, f, **kwargs):
        for point in self.points:
            self.drawpoint(f, point[0], point[1], point[2], point[3], **kwargs)
    ...
    def drawpoint(self, f, lat, lon, color, title='unknown', **kwargs):
        opened_info_window = kwargs.get('opened_info_window', False)
        f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' % (lat, lon))
        f.write('\t\tvar img = new google.maps.MarkerImage(\'%s\',null, null, null, new google.maps.Size(24, 24));\n' % (
        ...
        #f.write('\t\tinfowindow.setContent(\''+title+'\');\n')
        #f.write('\t\tinfowindow.open(map,marker);\n')

        f.write('\t\tgoogle.maps.event.addListener(marker,\'click\', (function(marker,content,infowindow){ \n');
