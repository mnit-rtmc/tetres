...
def run(route, prd, **kwargs):
    ...
    cm_results = [res.clone() for res in us_results]
    for ridx, res in enumerate(cm_results):
        cm_results[ridx].data = [0] * len(prd.get_timeline())
...
def sheet_writer(wb, sheet, results, r, n_data, rows_before_data, rows_after_data, **kwargs):
    ...
    tcmp_row = tcm_row + 1
    sheet.write_string(tcm_row, 0, 'TCM')
    sheet.write_string(tcmp_row, 0, 'TCM(%)')
    start_col = 1
    ...
    tcm_col = n_nodes + 1
    tcmp_col = tcm_col + 1
    sheet.write_string(0, tcm_col, 'TCM')
    sheet.write_string(0, tcmp_col, 'TCM(%)')
...
def post_book_writer(wb, results, r, **kwargs):
    ...
    sheet = wb.add_worksheet('Congested Miles')
    sheet.write_column(row=1, col=0, data=timeline + ['TCM'])
