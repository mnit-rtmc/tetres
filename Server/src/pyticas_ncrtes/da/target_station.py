...
class TargetStationDataAccess(object):
    def __init__(self, **kwargs):
        self.da_base = DataAccessBase(model.TargetStation, itypes.TargetStationInfo, **kwargs)
    ...
    def years(self):
        ...
        """
        ys = []
        for tsi in self.da_base.list(group_by='winterseason_id'):
            if tsi._winterseason.year not in ys:
                ys.append(tsi._winterseason.year)
        return sorted(ys)

    def list(self):
        """
        :rtype: list[itypes.TargetStationInfo]
        """
    ...
    def list_by_year(self, year, **kwargs):
        ...
        qry = self.da_base.session.query(model.TargetStation).filter(
            model.TargetStation._winterseason.has(year=year))
    ...
    def list_by_corridor_name(self, year, corridor_name, **kwargs):
        ...
        qry = self.da_base.session.query(model.TargetStation).filter(
            model.TargetStation._winterseason.has(year=year)).filter(model.TargetStation.corridor_name == corridor_name)
    ...
    def get_by_station_id(self, year, station_id, **kwargs):
        ...
        item = (self.da_base.session.query(model.TargetStation)
                .filter(model.TargetStation._winterseason.has(year=year))
                .filter(model.TargetStation.station_id == station_id).first())
