...
def write(normal_filepath, wetnormal_filepath, name, edata_list):
    """

    :type normal_filepath: str
    :type wetnormal_filepath: str
    :type name: str
    :type edata_list: list[pyticas_ncrtes.core.etypes.ESTData]
    """
    logger = getLogger(__name__)
    logger.debug('Writing ratio contour chart image of %s' % name)
    ...
    loaded_edatas = [ edata for edata in edata_list if edata.ks is not None and any(edata.ks) ]
    no_data = np.array([None]*len(loaded_edatas[0].ks))
    for sidx in range(0, n_stations):

        edata = nonon_edata_list[sidx]

        if edata is None or edata.normal_ratios is None or not any(edata.normal_ratios):
            continue
        ...
        if sidx == 0:
            if edata.normal_func:
                c_normal_ratio_data.append(edata.normal_ratios * 100)
            else:
                c_normal_ratio_data.append([None]*len(edata.us))

            if edata.wn_ffs:
                c_wetnormal_ratio_data.append(edata.wn_sratios * 100)
            else:
                c_wetnormal_ratio_data.append(no_data)
            c_y_labels.append('')
        ...
        if edata_next:
            ...
            for nidx in range(n_half):
                if edata_next.normal_func:
                    c_normal_ratio_data.append(edata_next.normal_ratios * 100)
                else:
                    c_normal_ratio_data.append([None]*len(edata_next.us))

                if edata_next.wn_ffs is not None:
                    c_wetnormal_ratio_data.append(edata_next.wn_sratios * 100)
                else:
                    c_wetnormal_ratio_data.append(no_data)
                c_y_labels.append('')
                c_corridor_ids.append('')
    ...
    title = 'Speed Level(%) vs. Normal Condition at {0}'.format(name)
    _contour(title, name, timeline, c_normal_ratio_data, c_recovery_points, c_y_labels, c_corridor_ids, c_snow_route, normal_filepath)

    title = 'Speed Level(%) vs. Wet-Normal Condition at {0}'.format(name)
    _contour(title, name, timeline, c_wetnormal_ratio_data, c_recovery_points, c_y_labels, c_corridor_ids, c_snow_route, wetnormal_filepath)


def _contour(title, name, timeline, range_data, recovery_points, y_labels, corridor_ids, snow_routes, file_path):
    """
    ...
    levels = MaxNLocator(nbins=12).tick_values(40, 100)
    ...
    cb = plt.colorbar(CS, cax=cbaxes, ticks=[0, 50, 60, 70, 80, 90, 100])
