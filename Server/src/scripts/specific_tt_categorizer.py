...
if __name__ == '__main__':
    ...
    if res.lower() not in ['y', 'ye', 'yes']:
        ...
        exit(1)
    categorizers = input(
        "Enter the space separated names of categorizers [Example: weather incident workzone specialevent snowmgmt]: ")
    categorizers = [categorizer.strip() for categorizer in categorizers.split()]
    from pyticas_tetres.systasks import initial_data_maker

    try:
        initial_data_maker.specific_tt_categorizer(sdate, edate, categorizers, db_info=dbinfo.tetres_db_info())
